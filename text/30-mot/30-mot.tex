...
\end{align}
with the Landé factor $g$, the Bohr magneton $\mu_\text{B}$, the magnetic field $B$ and the magnetic quantum number $m$. Applying a magnetic gradient, the transition frequencies between two magnetic sublevels $m$ and $m'$ becomes position-dependent:
\begin{equation}
    \begin{split}\label{eq:omega_transition_zeeman}
      \omega_\text{transition}(r) &= \omega_\text{transition} + \frac{\Delta E(r)}{\hbar} \\  
      &= \omega_\text{transition} + \frac{g \mu_\text{B}}{\hbar} \dv{B}{r} r \equiv \omega_\text{transition} + \beta r
    \end{split}
\end{equation}
...
\end{figure}
    ...
    \item[h] The slimmed-down light is then coupled through the first acousto-optic modulator operated at \SI{+78}{\mega\hertz}. The zeroth order of the outgoing beam is blocked at i. The power efficiency of this acousto-optic modulator single pass is \todo{add AOM efficiency}. Thanks to the telescope, the beam diameter at this point is significantly smaller than the specified aperture.
    ...
    \item[p, q] The cooler (q) and repumper (p) paths each contain an acousto-optic modulator operated at \SI{+114}{\mega\hertz} and \SI{-114}{\mega\hertz} respectively. Thanks to the refocusing lens (k), the beam diameter approximately matches the modulators' apertures. The power efficienies are \todo{add AOM efficiencies} for the cooler and for the repumper.
...
\begin{table}
    ...
    \begin{subtable}{\textwidth}
        ...
        \begin{tabular}{llc}
            \toprule
            \textbf{recombination} & \textbf{markers} (fig.~\ref{fig:2d_mot_setup_photo}) & \textbf{percentage} \\
            \toprule
            cooler & q - r & \SI{51}{\percent} \\
            repumper & p - r & \SI{47}{\percent} \\
            \bottomrule
        \end{tabular}
        \caption{Recombination ratios on polarizing beam splitter cube r}
    ...
    \begin{subtable}{\textwidth}
        ...
        \begin{tabular}{lllc}
            \toprule
            \multicolumn{2}{l}{\textbf{splitting}} & \textbf{markers} (fig.~\ref{fig:2d_mot_setup_photo}) & \textbf{ratio} \\
            \toprule 
            \multirow{3}{*}{branch A} & cooler \& repumper & s - t & \SI{50}{\percent} \\
            & cooler & q - t & \SI{52}{\percent} \\
            & repumper & p - t & \SI{49}{\percent} \\
            \midrule
            \multirow{3}{*}{branch B} & cooler \& repumper & s - u & \SI{51}{\percent} \\
            & cooler & q - u & \SI{54}{\percent} \\
            & repumper & p - u & \SI{46}{\percent} \\
            \bottomrule
        \end{tabular}
        \caption{Splitting ratios on polarizing beam splitter cube s}
...
\begin{align}\label{eq:fresnel}
    ...
    \end{split}
\end{align}
where the $\text{s}$ and $\text{p}$ components of the polarization have been mapped to the $\KetText{H}$ and $\KetText{V}$ basis vectors respectively.\todo{Check values and propagate if they were wrong}
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        \caption{Profile of the magnetic gradient along the coil axis}
        \label{fig:mot_gradient_axis}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \caption{Profile of the magnetic gradient along a radial axis}        
        \label{fig:mot_gradient_radial}
    \end{subfigure}
    \caption{Simulated maximum magnetic field gradients in the 3-dimensional magnetic optical trap for a current of \SI{400}{\ampere}: The dimensions of the trap beams are added for orientation
    \todo[inline]{add image}
    }
...
\todo{MOT extent and volume in different directions}
\todo{$v_\text{max}$}
