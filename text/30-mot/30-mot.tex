...
\begin{figure}
    \centering
    \caption{Simulated magnetic field gradients in the 3-dimensional magnetic optical trap for different currents along the recapturing path ($x$ axis). The dimensions of the trap beams are added for orientation. Details about the simulation are described in chapter \ref{ch:simulation}.
    \todo[inline]{add image}
    }
...
\todo{consider smaller beams due to reflection losses}
...
\begin{figure}
    ...
    \todo[inline]{consider smaller beams due to reflection losses}
    }
    \label{fig:mot_extent}
\end{figure}


\todo{relate detuning to trap size  using gradients, use Tiecke expression from far above}
