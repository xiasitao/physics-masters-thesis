...
\begin{figure}    
    \centering
    \caption{Schematic of momentum transfer from the laser opposite the atom movement direction. The momentum gain through the atom decaying averages to zero.}\label{fig:light_scattering_moment_transfer}
\end{figure}
...
Figure~\ref{fig:scattering_force_vs_velocity} shows how the scattering force depends on the velocity of the atoms. The force is maximal for velocities where Doppler shift $\vec k \vec v$ cancels out the laser detuning $\delta_\text{laser} = \omega_\text{laser} - \omega_\text{transition}$. Deceleration of atoms happens with red laser detuning $\delta_\text{laser} < 0$ as the light is resonant for atoms moving opposite the laser propagation direction. These atoms sense the light scattering force against their direction of movement; atoms moving in the other direction do not sense the scattering force (see figure~\ref{fig:scattering_force_detuning}).
...
If, however, due to a non-zero matrix element for this transition, the atom is excited into the $\Ket{J = J_\text{e}, F = F_\text{g}}$ state, it can also fall back into the $\Ket{J = J_\text{e}, F = I + J_\text{g} - 1}$ state. In this case, it would be lost for the cooling process as this state is not addressed by the cooling transition. To combat this, a repumper laser beam is shone in in addition to the cooler beam. This beam addresses the transition $\Ket{J = J_\text{g}, F = I + J_\text{g} - 1} \rightarrow \Ket{J = J_\text{g} + 1, F = I + J_\text{g}}$ from where they can decay back into the ground state of cool the cooling transition which brings them these back into the cooling cycle~\cite{metcalf_laser_1999}.
