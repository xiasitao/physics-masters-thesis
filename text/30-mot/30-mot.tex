...
\begin{equation}
    \begin{split}\label{eq:omega_transition_zeeman}
      ...
      &= \omega_\text{transition} + \frac{g \mu_\text{B}}{\hbar} \dv{B}{q} q \equiv \omega_\text{transition} + \beta q
    \end{split}
\end{equation}
...
\begin{equation}\label{eq:mot_force}
    \begin{split}
        F_\text{MOT} &= F_{\sigma^+}(\omega_\text{laser} - \omega_\text{transition}(q) - kv \cos \theta) + F_{\sigma^-}(\omega_\text{laser} - \omega_\text{transition}(q) + kv \cos \theta)\\
        &= -2 \pdv{F}{\omega} k\cos \theta v + 2 \pdv{F}{\omega_0} \beta q  \equiv - \alpha \cos \theta v - \frac{\alpha \beta}{k} q
    \end{split}
\end{equation}
...
\begin{itemize}
    ...
    \item Additionally, one can estimate a maximum velocity $v_\text{max, capture}$ such that the atoms entering the trap are decelerated to standstill within the length $2r_\text{trap}$ of the trap. This would require that their kinetic energy $\frac{1}{2}mv^2$ be dissipated by the cooling force performing the mechanical work $\int F_\text{MOT}(q) \dd{q} = \int (-\alpha \cos \theta v(q) - \frac{\alpha\beta}{k}q) \dd{q} = - \int \alpha \cos \theta v(q) \dd{q}$ (see \cite{nn_mot_2018}\todo{find better citation} and \eqref{eq:mot_force}):
    \begin{align}
            v_\text{max, capture} 
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \todo[inline]{remove any radii that are outside the 7mm (x axis) recapture range}
        }
...
\paragraph{Capture and Resonance Velocities}\todo{$v_\text{max}$, talk about shallow angles again}
