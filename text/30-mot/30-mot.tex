...
\todo{Hint to typical ranges of capture velocities and radii, according to these typical parameters}
...
The necessary output beams for the traps and the gray molasses are created from the two input beams on the D$_2$ and the D$_1$ frequency as schematized in figure~\ref{fig:laser_setup_schematic}.\todo{Shortly describe what these beams need to have: cooler and repumper, 2D MOT steady, 3D MOT + gray molasses power- and frequency-tunable}
...
\subsection*{Implementation of the Laser Setup for the two-dimensional Magneto-optical Trap}\todo{Shorten dramatically!}
...
\begin{figure}
    ...
    \caption{Photograph of the part of the optics setup providing light for the two-dimensional magneto-optical trap. The beam path and reference points have been added for orientation.
    \todo[inline]{Add important couplings to the setup: AOMs and total fiber couplings}
    }
...
\end{figure}
    ...
    \item[i] A half-wave plate sets the splitting ratio between light for the push beam (m) and light for the two-dimensional magneto-optical trap.
    ...
    \item[l] The light is split into a few \si{\milli\watt} for the push beam (m) and the rest for the two-dimensional magneto-optical trap.
...
Refer to the appendix chapter~\ref{ch:power_losses} for a more detailed listing of the  measured remaining power levels after each dissipating element, the measured cooler and repumper recombination ratios (r), and the measured splitting ratios at the before the optical fibers (s).
...
\todo{Add closing statement: cite atom numbers and loading rates from 1-2 papers, and conclude with something about experimentally testing and verifying}
...
\todo[color=green]{Ch 3 proofreading run 2 until here}