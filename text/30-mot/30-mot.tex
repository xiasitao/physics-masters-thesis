...
\end{figure}
    ...
    \item[t, u] The two resulting beams are coupled into fibers guiding it to the experiment chamber. In each beam, a pair of a quarter- and half-wave plate allow for arbitrary corrections of the polarization if necessary. The coupling efficiencies are \SI{84}{\percent} for fiber A (t) and \SI{74}{\percent} for fiber B (u) for both cooler and repumper (see also table~\ref{tab:power_loss_table}).
\end{itemize}
...
\todo[]{average intensities}
\todo{$v_\text{max}$, talk about shallow angles again}
