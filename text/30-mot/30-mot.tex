...
The outcoupling stage consists of optical elements for collimating, polarization-cleaning, and polarization-preparing the light, mounted in an optical cage system. A schematic of the stage is shown in figure~\ref{fig:outcoupling_stage_schematic}
...
In order to reach right or left circular polarizations within the glass cell, these polarization-dependent losses need to be pre-compensated and find the following polarizations required before glass cell:
\begin{align}
...
In a mock setup a specimen fused silica glass plate of thickness \SI{4.04}{\milli\meter} was placed at an angle of \SI{60}{\degree} with respect to a \SI{671}{\nano\meter} laser beam in order to verify these polarization presets. Using a polarization analyser, the polarization was set before the glass plate and measured after the glass plate. The results are displayed in table~\ref{tab:polarization_first_pass} and show that the aforementioned preset polarizations indeed lead to circular polarization in the glass cell.
...
\begin{table}
    \begin{tabularx}{\textwidth}{rcc}
        ...
        \midrule
        $\KetText{R}$ & $(0.67, 0.01 + 0.75i)$ & $(0.71, 0.00 - 0.70i)$ \\
        $\KetText{L}$ & $(0.63, 0.03 + 0.78i)$ & $(0.71, 0.00 + 0.70i)$ \\
        \bottomrule
    \end{tabularx}
    \caption{Experimental verification of the preset polarization for the first pass through the glass cell.}
