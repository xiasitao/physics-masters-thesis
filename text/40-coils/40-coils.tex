...
\begin{figure}
    ...
    \caption{Position of the Feshbach coils with respect to the glass cell and the objectives: The objectives (\SI{44}{\milli\meter} diameter) which are positioned below and above the glass cell are enclosed by the two coils, which sets the minimum distance between them. The innermost coil wires have a distance of \SI{55}{\milli\meter}.}
...
When the atoms trapped in the glass cell are exposed to the homogeneous magnetic field, they experience an energy shift resulting from the interaction of their magnetic moment $\vec \mu$ with the field $\vec B$ which can lead to trapping or anti-trapping effects~\cite{pritchard_cooling_1983,gehm_properties_2003, hagemann_setup_2020}:
\begin{align}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.47\textwidth}
        ...
        \caption{3D sketch of the Feshbach coil arrangement, modelled as individual circular windings (generated by \textit{magpylib}~\cite{noauthor_magpylibmagpylib_nodate})}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \end{pgfpicture}
        \caption{Slices through the field landscape of the Feshbach field along the coil axis and along the cross-axis (radial) at $I = \SI{400}{\ampere}$}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \end{pgfpicture}
        \caption{Zoom into the center area (field given as difference from the center value), revealing the different curvatures in axial and radial direction}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \end{pgfpicture}
        \caption{Slices through the field landscape of the gradient field along the coil axis and along the cross-axis (radial) at $I = \SI{+-400}{\ampere}$}
...
\subsection*{Field Landscape Measurement}
...
These measured values $B$ were fitted to the following model for the on-axis field of \SI{30}{} spatially separated wire loops with the current $I$\footnote{Although the current was set on the power supply and verified using a multimeter, adding it as a fit parameter greatly improved the quality of the fit.}, the radius $r$, and the center position $y_0$ as free parameters~\cite{demtroder_statische_2013}:
...
\begin{figure}
    \centering
    \includegraphics[]{\imagepath/coil_measurement_setup/coil_measurement_setup.pdf}
...
\begin{table}
    ...
    \begin{tabular}{cccc}
        \toprule
        \textbf{pancake} & \textbf{coil} & \textit{\textbf{y}}$_\textbf{max}$ & \textit{\textbf{B}}$_\textbf{max}$ \\
        \toprule
...
\begin{figure}
    ...
    \begin{minipage}{\textwidth}
        ...
        \begin{tabular}{rccc}
            ...
            \bottomrule
        \end{tabular}
        \captionof{table}{Expected optimal positions of the coils \textit{Gauss} and \textit{Tesla} for minimum gradient and minimum trap frequency (see figure~\ref{fig:position_optimization})}
...
\begin{figure}
    ...
    \end{pgfpicture}
    \caption{Expected field landscape sampled from the field landscape model \eqref{eq:on_axis_field_model} for \textit{Gauss} (left) and \textit{Tesla} (right) with optimized positions $y_{0, \textit{Gauss}} = \SI{-41.0}{\milli\meter}$ and $y_{0, \textit{Gauss}} = \SI{41.5}{\milli\meter}$ and optimized currents $I_\textit{Gauss} = \SI{400.0}{\ampere}$, $I_\textit{Tesla} = \SI{396.5}{\ampere}$. The resulting field landscape has a gradient of \SI{-0.07}{\gauss\per\centi\meter} and a trap frequency of \SI{0.63}{\hertz}.}
    \label{fig:optimized_projected_landscape}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[]{\imagepath/coil_holder/coil_holder.pdf}
