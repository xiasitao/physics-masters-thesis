...
\begin{table}
    ...
    \begin{tabular}{rcc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Operational properties of the single Feshbach coils determined using the \text{coil simulation library}}
...
In this section, on-axis field measurements are used to determine deviations of the Feshbach coils from the specification. Considering these imperfections, optimal operation currents and positions for the coils to achieve minimal trap frequencies and minimal gradients are inferred.


\subsection*{Fields Landscape Measurements}
...
\end{align}
with the position $y_{ac} = y_0 + (a - 2.5) w$ and the radius $r_{ac} = r_0 + (c-2)w$ of a winding with on-axis index $a$, cross-axis index $c$, and a winding spacing $w$.
